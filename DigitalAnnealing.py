                        ...
                        for approximation_type in approximation_types:
                            ...
                            for number_iterations in iterations_list:
                                ...
                                while sample_index < max_num_samples and total_annealing_time < timeout_in_ms:
                                    result_path = result_path_prefix + '/digital_annealing/benchmark_queries/' + benchmark + '/' + query + '/' + approximation_type + '_approximation' + '/thres_config_' + str(ap) + '/penalty_scaling_' + str(penalty_scaling) + '/' + da_algorithm + '/' + str(number_iterations) + '_iterations/' + str(number_runs) + '_shots/' + 'sample_' + str(sample_index)
                                    ...
                                    thres_path = result_path_prefix + '/digital_annealing/benchmark_queries/' + benchmark + '/' + query + '/' + approximation_type + '_approximation' + '/thres_config_' + str(ap)
                ...
                for approximation_type in approximation_types:
                    for penalty_scaling in penalty_scalings:
                        ...
                        for number_iterations in iterations_list:
                            ...
                            thres_config_path = data_path_prefix + '/digital_annealing/benchmark_queries/' + benchmark + '/' + query + '/' + approximation_type + '_approximation'
                            ...
                            for thres_config in thres_configs:
                                ...
                                for s in range(max_num_samples):
                                    ...
                                    data_path = data_path_prefix + '/digital_annealing/benchmark_queries/' + benchmark + '/' + query + '/' + approximation_type + '_approximation/' + thres_config + '/penalty_scaling_' + str(penalty_scaling) + '/' + da_algorithm + '/' + str(number_iterations) + '_iterations/' + str(number_runs) + '_shots/' + data_path_suffix 
                        ...
                        for l in range(len(approximation_precisions)):
                            ...
                            for approximation_type in approximation_types:
                                ...
                                for number_iterations in iterations_list:
                                    ...
                                    sample_index = 0
                                    while sample_index < max_num_samples and total_annealing_time < timeout_in_ms:
                                        result_path = result_path_prefix + '/digital_annealing/synthetic_queries/' + graph_type_string + '/' + relations_string + '/' + str(j) + '/' + approximation_type + '_approximation' + '/thres_config_' + str(ap) + '/penalty_scaling_' + str(penalty_scaling) + '/' + da_algorithm + '/' + str(number_iterations) + '_iterations/' + str(number_runs) + '_shots/' + 'sample_' + str(sample_index)

                                        raw_solutions, execution_time, anneal_time = solve_problem(fujitsu_qubo, da_algorithm=da_algorithm, number_runs=number_runs, number_iterations=number_iterations)
                                        solutions = parse_solutions_for_serialisation(raw_solutions)
                                        total_annealing_time = total_annealing_time + (execution_time * 1000)
                                        save_data([solutions, float(execution_time), float(anneal_time)], result_path, "response.txt") 
                                        thres_path = result_path_prefix + '/digital_annealing/synthetic_queries/' + graph_type_string + '/' + relations_string + '/' + str(j) + '/' + approximation_type + '_approximation' + '/thres_config_' + str(ap)
                                        if not os.path.exists(thres_path + '/thres_config.txt'):
                                            save_data(thres, thres_path, 'thres_config.txt')
                                        sample_index = sample_index + 1
                    ...
                    for approximation_type in approximation_types:
                        for penalty_scaling in penalty_scalings:
                            ...
                            for number_iterations in iterations_list:
                                ...
                                thres_config_path = data_path_prefix + '/digital_annealing/synthetic_queries/' + problem_path_main + '/' + approximation_type + '_approximation'
                                if not os.path.exists(thres_config_path):
                                    continue
                                thres_configs = os.listdir(path=thres_config_path)
                                for thres_config in thres_configs:
                                    ...
                                    for s in range(max_num_samples):
                                        data_path_suffix = 'sample_' + str(s)
                                        data_path = data_path_prefix + '/digital_annealing/synthetic_queries/' + problem_path_main + '/' + approximation_type + '_approximation/' + thres_config + '/penalty_scaling_' + str(penalty_scaling) + '/' + da_algorithm + '/' + str(number_iterations) + '_iterations/' + str(number_runs) + '_shots/' + data_path_suffix 
                                        if not os.path.exists(data_path):
                                            continue
                                        response = load_data(data_path, "response.txt")
                                        if response is None:
