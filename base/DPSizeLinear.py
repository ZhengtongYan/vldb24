...
def conduct_benchmark_experiments(modes, problem_path_prefix, result_path_prefix, timeout_in_ms = 60000):
    ...
    for benchmark in benchmarks:
        ...
        for query in queries:
            ...
            raw_query = load_data(problem_path_prefix + '/benchmarks_raw/' + benchmark, query)
            ...
            for mode in modes:
                if mode == "allow_cross_products":
                    ...
                    save_data(result, result_path_prefix + '/benchmarks/' + benchmark + '/' + query, 'dpsizelinearCP.json')
                    ...
                    if len(join_order) > 0:
                        result.append({"time": opt_time_in_ms, "join_order": join_order, "costs:": int(costs)})
                    save_data(result, result_path_prefix + '/benchmarks/' + benchmark + '/' + query, 'dpsizelinearNCP.json')
                else:
                    continue
    ...
    for graph_type_string in graph_types:
        ...
        for relations_string in relations:
            ...
            for j in problems:
                ...
                result_path = result_path_prefix + '/synthetic/' + problem_path_main
                for mode in modes:
                    if mode == "allow_cross_products":
                        start = time.time()
...
if __name__ == "__main__":
    ...
    result_path_prefix = 'Experiments/ResultsBenchmarks'
