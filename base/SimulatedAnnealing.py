                    ...
                    for approximation_type in approximation_types:
                        ...
                        for number_iterations in iterations_list:
                            ...
                            while sample_index < max_num_samples and total_annealing_time < timeout_in_ms:
                                result_path = result_path_prefix + '/sa/benchmarks/' + benchmark + '/' + query + '/' + approximation_type + '_approximation/' + '/thres_config_' + str(ap) + '/penalty_scaling_' + str(penalty_scaling) + '/' + str(number_iterations) + '_iterations/' + str(number_runs) + '_shots/' + 'sample_' + str(sample_index)
                                ...
                                DataExport.compress_and_save_data([solutions, float(opt_time)], result_path, "response.txt") 
                                thres_path = result_path_prefix + '/sa/benchmarks/' + benchmark + '/' + query + '/' + approximation_type + '_approximation/' + '/thres_config_' + str(ap)
            ...
            for approximation_type in approximation_types:
                for penalty_scaling in penalty_scalings:
                    ...
                    for number_iterations in iterations_list:
                        ...
                        result_file = result_path +  '/sa_' + approximation_type + '.json'
                        ...
                        thres_config_path = data_path_prefix + '/sa/benchmarks/' + benchmark + '/' + query + '/' + approximation_type + '_approximation'
                        ...
                        for thres_config in thres_configs:
                            ...
                            for s in range(max_num_samples):
                                ...
                                data_path = data_path_prefix + '/sa/benchmarks/' + benchmark + '/' + query + '/' + approximation_type + '_approximation/' + thres_config + '/penalty_scaling_' + str(penalty_scaling) + '/' + str(number_iterations) + '_iterations/' + str(number_runs) + '_shots/' + data_path_suffix 
                    ...
                    for l in range(len(approximation_precisions)):
                        ...
                        for approximation_type in approximation_types:
                            ...
                            for number_iterations in iterations_list:
                                ...
                                sample_index = 0
                                while sample_index < max_num_samples and total_annealing_time < timeout_in_ms:
                                    result_path = result_path_prefix + '/sa/synthetic/' + graph_type_string + '/' + relations_string + '/' + str(j) + '/' + approximation_type + '_approximation/' + '/thres_config_' + str(ap) + '/penalty_scaling_' + str(penalty_scaling) + '/' + str(number_iterations) + '_iterations/' + str(number_runs) + '_shots/' + 'sample_' + str(sample_index)
                                    solutions, opt_time = solve_problem(qubo, number_runs=number_runs, number_iterations=number_iterations)
                                    total_annealing_time = total_annealing_time + opt_time
                                    DataExport.compress_and_save_data([solutions, float(opt_time)], result_path, "response.txt") 
                                    thres_path = result_path_prefix + '/sa/synthetic/' + graph_type_string + '/' + relations_string + '/' + str(j) + '/' + approximation_type + '_approximation/' + '/thres_config_' + str(ap)
                                    if not os.path.exists(thres_path + '/thres_config.txt'):
                                        save_data(thres, thres_path, 'thres_config.txt')
                                    sample_index = sample_index + 1
                ...
                for approximation_type in approximation_types:
                    for penalty_scaling in penalty_scalings:
                        ...
                        for number_iterations in iterations_list:
                            ...
                            thres_config_path = data_path_prefix + '/sa/synthetic/' + problem_path_main + '/' + approximation_type + '_approximation'
                            if not os.path.exists(thres_config_path):
                                continue
                            thres_configs = os.listdir(path=thres_config_path)
                            for thres_config in thres_configs:
                                ...
                                for s in range(max_num_samples):
                                    data_path_suffix = 'sample_' + str(s)
                                    data_path = data_path_prefix + '/sa/synthetic/' + problem_path_main + '/' + approximation_type + '_approximation/' + thres_config + '/penalty_scaling_' + str(penalty_scaling) + '/' + str(number_iterations) + '_iterations/' + str(number_runs) + '_shots/' + data_path_suffix 
                                    if not os.path.exists(data_path):
                                        continue
                                    response = DataExport.load_compressed_data(data_path, "response.txt")
                                    if response is None:
